...
<div class="container-fluid px-4">
    <div class="mb-1 hstack gap-2">
        <h2 class="mt-3">Editar Meta de Desempenho</h2>
    ...
    <div class="card mb-4 border-light shadow">
        <div class="card-header">
            <span><i class="fas fa-bullseye me-2"></i>Editar Meta de Desempenho (OKR)</span>
        </div>
        <div class="card-body">
            <?php include './app/adms/Views/partials/alerts.php'; ?>
            
            <form action="" method="POST" class="row g-3">
                <input type="hidden" name="csrf_token" value="<?php echo CSRFHelper::generateCSRFToken('form_update_performance_goal'); ?>">
                ...
                <div class="col-md-6">
                    ...
                    <select name="employee_id" id="employee_id" class="form-select" required>
                        ...
                        <?php foreach (($this->data['employees'] ?? []) as $employee): ?>
                            <option value="<?= $employee['id'] ?>" <?= ($employee['id'] == ($goal['employee_id'] ?? 0)) ? 'selected' : '' ?>>
                ...
                <div class="col-md-6">
                    ...
                    <select name="goal_type" id="goal_type" class="form-select" required>
                        <option value="individual" <?= ($goal['goal_type'] ?? '') == 'individual' ? 'selected' : '' ?>>Individual</option>
                        <option value="team" <?= ($goal['goal_type'] ?? '') == 'team' ? 'selected' : '' ?>>Equipe</option>
                        <option value="company" <?= ($goal['goal_type'] ?? '') == 'company' ? 'selected' : '' ?>>Empresa</option>
                    </select>
                </div>
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="goal_title" id="goal_title" class="form-control" 
                           value="<?= htmlspecialchars($goal['goal_title'] ?? '') ?>" required>
                </div>
                
                <div class="col-12">
                    <label for="goal_description" class="form-label">Descrição</label>
                    <textarea name="goal_description" id="goal_description" class="form-control" rows="3"><?= htmlspecialchars($goal['goal_description'] ?? '') ?></textarea>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="target_value" id="target_value" class="form-control" 
                           step="0.01" value="<?= htmlspecialchars($goal['target_value'] ?? '') ?>" placeholder="Ex: 100">
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="current_value" id="current_value" class="form-control" 
                           step="0.01" value="<?= htmlspecialchars($goal['current_value'] ?? 0) ?>" placeholder="0">
                </div>
                
                <div class="col-md-4">
                    <label for="unit" class="form-label">Unidade de Medida</label>
                    <input type="text" name="unit" id="unit" class="form-control" 
                           value="<?= htmlspecialchars($goal['unit'] ?? '') ?>" placeholder="Ex: %, unidades, R$, etc.">
                </div>
                
                <div class="col-md-4">
                    <label for="deadline" class="form-label">Prazo</label>
                    <input type="date" name="deadline" id="deadline" class="form-control" 
                           value="<?= !empty($goal['deadline']) ? date('Y-m-d', strtotime($goal['deadline'])) : '' ?>">
                </div>
                
                <div class="col-md-4">
                    <label for="weight" class="form-label">Peso (Importância)</label>
                    <input type="number" name="weight" id="weight" class="form-control" 
                           step="0.1" min="0.1" max="10" value="<?= htmlspecialchars($goal['weight'] ?? 1.0) ?>" placeholder="1.0">
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" id="status" class="form-select">
                        ...
                        <option value="achieved" <?= ($goal['status'] ?? '') == 'achieved' ? 'selected' : '' ?>>Alcançada</option>
                        <option value="failed" <?= ($goal['status'] ?? '') == 'failed' ? 'selected' : '' ?>>Não Alcançada</option>
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="progress_percentage" id="progress_percentage" class="form-control" 
                           min="0" max="100" value="<?= htmlspecialchars($goal['progress_percentage'] ?? 0) ?>" placeholder="0">
                ...
                <div class="col-12">
                    <div class="d-flex gap-2">
                        ...
                        <a href="<?php echo $_ENV['URL_ADM']; ?>view-performance-goal/<?= $goal['id'] ?? '' ?>" class="btn btn-secondary">
