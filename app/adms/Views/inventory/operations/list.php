...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    ...
    <div>
      <?php if (!empty($this->data['buttonPermission']) && in_array('CreateInventoryOperation', $this->data['buttonPermission'])): ?>
        <a href="<?= $_ENV['URL_ADM'] ?>create-inventory-operation" class="btn btn-sm btn-success">Cadastrar</a>
      <?php endif; ?>
    </div>
  </div>
  <div class="card-body">
    <form class="row g-2 mb-3" method="get">
      <input type="hidden" name="url" value="list-inventory-operations">
      ...
      <div class="col-12 col-md-4 d-flex gap-2">
        ...
        <a class="btn btn-outline-secondary" href="<?= $_ENV['URL_ADM'] ?>list-inventory-operations">Limpar</a>
      </div>
    </form>
      ...
      <table class="table table-striped align-middle w-100">
        <thead class="thead-green">
          <tr>
            <th>Código</th>
            <th>Nome</th>
            <th class="text-end">Custo/hora padrão</th>
          ...
          <?php foreach (($this->data['rows'] ?? []) as $row): ?>
            <tr>
              ...
              <td class="text-end">
                ...
                <?php endif; ?>
                <?php if (!empty($this->data['buttonPermission']) && in_array('DeleteInventoryOperation', $this->data['buttonPermission'])): ?>
                  <form method="post" class="d-inline">
                    <input type="hidden" name="csrf_token" value="<?= \App\adms\Helpers\CSRFHelper::generateCSRFToken('form_delete_inventory_operation') ?>">
