...
$csrf_token = CSRFHelper::generateCSRFToken('form_delete_sst_epi_entregas');
$filtersId = 'sstFiltersEpiEntrega';
?>
<div class="container-fluid px-4">
    <div class="mb-1 hstack gap-2">
        <h2 class="mt-3 mobile-hide-page-title"><i class="fas fa-hand-holding me-2"></i><?= htmlspecialchars('Entregas de EPI') ?></h2>
        <ol class="breadcrumb mb-3 ms-auto mobile-hide-breadcrumb">
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>dashboard">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>sst-dashboard">SST</a></li>
            <li class="breadcrumb-item"><?= htmlspecialchars('Entregas de EPI') ?></li>
        </ol>
    </div>
    <div class="card mb-4 border-light shadow">
        <div class="card-header hstack gap-2">
            <span>Listar</span>
            <span class="ms-auto">
                <?php if (in_array('SstCreateEpiEntrega', $perms)): ?>
                    <a href="<?= $_ENV['URL_ADM']; ?>sst-create-epi-entrega" class="btn btn-success btn-sm"><i class="fa-regular fa-square-plus"></i> Cadastrar</a>
                <?php endif; ?>
            </span>
        </div>
        <div class="card-body">
            ...
            <select name="adms_user_id" id="adms_user_id" class="form-select form-select-sm">
                ...
                <?php endforeach; ?>
                    ...
                    <div class="col-12 col-sm-auto d-flex gap-2">
                        ...
                        <a href="<?= $_ENV['URL_ADM']; ?>sst-list-epi-entregas" class="btn btn-secondary btn-sm">Limpar</a>
                    </div>
                </form>
            </div>
...
<td><?= formatCellValue('epi_nome', $item['epi_nome'] ?? null) ?></td>
<td><?= formatCellValue('tipo_movimento', $item['tipo_movimento'] ?? null) ?></td>
<td><?= formatCellValue('data_movimento', $item['data_movimento'] ?? null) ?></td>

                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <?php if (in_array('SstViewEpiEntrega', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-view-epi-entrega/<?= $id ?>" class="btn btn-info btn-sm" title="Visualizar"><i class="fa-regular fa-eye"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstUpdateEpiEntrega', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-update-epi-entrega/<?= $id ?>" class="btn btn-warning btn-sm"><i class="fa-regular fa-pen-to-square"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstDeleteEpiEntrega', $perms)): ?>
                                            <form action="<?= $_ENV['URL_ADM']; ?>sst-delete-epi-entrega" method="POST" class="d-inline" onsubmit="return confirm('Excluir registro?');">
                ...
                <div class="d-block d-md-none">
                    <?php foreach ($this->data['items'] as $item):
                        $id = (int)($item['id'] ?? 0);
                        $canView = in_array('SstViewEpiEntrega', $perms);
                        $viewUrl = $_ENV['URL_ADM'] . 'sst-view-epi-entrega/' . $id;
                    ?>
                        <div class="card mb-2 shadow-sm"<?php if ($canView): ?> onclick="window.location.href='<?= $viewUrl ?>';" style="cursor:pointer;"<?php endif; ?>>
                            <div class="card-body py-2 px-3">
                                ...
                                <?php if (!empty($item['epi_nome'])): ?>
                                    <div class="small text-muted"><?= formatCellValue('epi_nome', $item['epi_nome']) ?></div>
                                <?php endif; ?>
                                ...
                                <div class="d-flex gap-1 mt-2 pt-2 border-top" onclick="event.stopPropagation();">
                                    ...
                                    <?php if (in_array('SstUpdateEpiEntrega', $perms)): ?><a href="<?= $_ENV['URL_ADM']; ?>sst-update-epi-entrega/<?= $id ?>" class="btn btn-outline-warning btn-sm flex-fill">Editar</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
