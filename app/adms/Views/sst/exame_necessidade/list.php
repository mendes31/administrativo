...
$csrf_token = CSRFHelper::generateCSRFToken('form_delete_sst_exame_necessidade');
$filtersId = 'sstFiltersExameNecessidade';
?>
<div class="container-fluid px-4">
    <div class="mb-1 hstack gap-2">
        <h2 class="mt-3 mobile-hide-page-title"><i class="fas fa-clipboard-list me-2"></i><?= htmlspecialchars('Necessidades de Exame') ?></h2>
        <ol class="breadcrumb mb-3 ms-auto mobile-hide-breadcrumb">
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>dashboard">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>sst-dashboard">SST</a></li>
            <li class="breadcrumb-item"><?= htmlspecialchars('Necessidades de Exame') ?></li>
        </ol>
    </div>
    <div class="card mb-4 border-light shadow">
        <div class="card-header hstack gap-2">
            <span>Listar</span>
            <span class="ms-auto">
                <?php if (in_array('SstCreateExameNecessidade', $perms)): ?>
                    <a href="<?= $_ENV['URL_ADM']; ?>sst-create-exame-necessidade" class="btn btn-success btn-sm"><i class="fa-regular fa-square-plus"></i> Cadastrar</a>
                <?php endif; ?>
            </span>
        </div>
        <div class="card-body">
            ...
            <div class="collapse d-md-block" id="<?= $filtersId ?>">
                <form method="get" class="row g-2 mb-3 align-items-end">
                    ...
                    <div class="col-12 col-sm-auto d-flex gap-2">
                        ...
                        <a href="<?= $_ENV['URL_ADM']; ?>sst-list-exame-necessidade" class="btn btn-secondary btn-sm">Limpar</a>
                    </div>
                </form>
            </div>
...
<td><?= formatCellValue('exame_nome', $item['exame_nome'] ?? null) ?></td>
<td><?= formatCellValue('periodicidade_meses', $item['periodicidade_meses'] ?? null) ?></td>

                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        
                                        <?php if (in_array('SstUpdateExameNecessidade', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-update-exame-necessidade/<?= $id ?>" class="btn btn-warning btn-sm"><i class="fa-regular fa-pen-to-square"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstDeleteExameNecessidade', $perms)): ?>
                                            <form action="<?= $_ENV['URL_ADM']; ?>sst-delete-exame-necessidade" method="POST" class="d-inline" onsubmit="return confirm('Excluir registro?');">
                ...
                <div class="d-block d-md-none">
                    <?php foreach ($this->data['items'] as $item):
                        $id = (int)($item['id'] ?? 0);
                        $canView = in_array('SstViewExameNecessidade', $perms);
                        $viewUrl = $_ENV['URL_ADM'] . 'sst-view-exame-necessidade/' . $id;
                    ?>
                        <div class="card mb-2 shadow-sm"<?php if ($canView): ?> onclick="window.location.href='<?= $viewUrl ?>';" style="cursor:pointer;"<?php endif; ?>>
                            <div class="card-body py-2 px-3">
                                ...
                                <div class="d-flex gap-1 mt-2 pt-2 border-top" onclick="event.stopPropagation();">
                                    ...
                                    <?php if (in_array('SstUpdateExameNecessidade', $perms)): ?><a href="<?= $_ENV['URL_ADM']; ?>sst-update-exame-necessidade/<?= $id ?>" class="btn btn-outline-warning btn-sm flex-fill">Editar</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
