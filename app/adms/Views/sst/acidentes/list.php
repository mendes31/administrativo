...
$csrf_token = CSRFHelper::generateCSRFToken('form_delete_sst_acidentes');
$filtersId = 'sstFiltersAcidente';
?>
<div class="container-fluid px-4">
    <div class="mb-1 hstack gap-2">
        <h2 class="mt-3 mobile-hide-page-title"><i class="fas fa-ambulance me-2"></i><?= htmlspecialchars('Acidentes e Incidentes') ?></h2>
        <ol class="breadcrumb mb-3 ms-auto mobile-hide-breadcrumb">
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>dashboard">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>sst-dashboard">SST</a></li>
            <li class="breadcrumb-item"><?= htmlspecialchars('Acidentes e Incidentes') ?></li>
        </ol>
    </div>
    <div class="card mb-4 border-light shadow">
        <div class="card-header hstack gap-2">
            <span>Listar</span>
            <span class="ms-auto">
                <?php if (in_array('SstCreateAcidente', $perms)): ?>
                    <a href="<?= $_ENV['URL_ADM']; ?>sst-create-acidente" class="btn btn-success btn-sm"><i class="fa-regular fa-square-plus"></i> Cadastrar</a>
                <?php endif; ?>
            </span>
        </div>
        <div class="card-body">
            ...
            <select name="status" id="status" class="form-select form-select-sm">
                <option value="">Todos</option><?php $sel = ($this->data['filters']['status'] ?? '') === 'Aberto' ? 'selected' : ''; ?>
<option value="Aberto" <?= $sel ?>>Aberto</option>
<?php $sel = ($this->data['filters']['status'] ?? '') === 'Em investigação' ? 'selected' : ''; ?>
<option value="Em investigação" <?= $sel ?>>Em investigação</option>
<?php $sel = ($this->data['filters']['status'] ?? '') === 'Encerrado' ? 'selected' : ''; ?>
<option value="Encerrado" <?= $sel ?>>Encerrado</option>
</select></div>
                    <div class="col-12 col-sm-auto d-flex gap-2">
                        <button type="submit" class="btn btn-primary btn-sm"><i class="fa fa-search"></i> Filtrar</button>
                        <a href="<?= $_ENV['URL_ADM']; ?>sst-list-acidentes" class="btn btn-secondary btn-sm">Limpar</a>
                    </div>
                </form>
            </div>
...
<td><?= formatCellValue('data_ocorrencia', $item['data_ocorrencia'] ?? null) ?></td>
<td class="text-center"><span class="badge bg-secondary"><?= htmlspecialchars($item['status'] ?? '') ?></span></td>

                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <?php if (in_array('SstViewAcidente', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-view-acidente/<?= $id ?>" class="btn btn-info btn-sm" title="Visualizar"><i class="fa-regular fa-eye"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstUpdateAcidente', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-update-acidente/<?= $id ?>" class="btn btn-warning btn-sm"><i class="fa-regular fa-pen-to-square"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstDeleteAcidente', $perms)): ?>
                                            <form action="<?= $_ENV['URL_ADM']; ?>sst-delete-acidente" method="POST" class="d-inline" onsubmit="return confirm('Excluir registro?');">
                ...
                <div class="d-block d-md-none">
                    <?php foreach ($this->data['items'] as $item):
                        $id = (int)($item['id'] ?? 0);
                        $canView = in_array('SstViewAcidente', $perms);
                        $viewUrl = $_ENV['URL_ADM'] . 'sst-view-acidente/' . $id;
                    ?>
                        <div class="card mb-2 shadow-sm"<?php if ($canView): ?> onclick="window.location.href='<?= $viewUrl ?>';" style="cursor:pointer;"<?php endif; ?>>
                            <div class="card-body py-2 px-3">
                                ...
                                <div class="d-flex gap-1 mt-2 pt-2 border-top" onclick="event.stopPropagation();">
                                    ...
                                    <?php if (in_array('SstUpdateAcidente', $perms)): ?><a href="<?= $_ENV['URL_ADM']; ?>sst-update-acidente/<?= $id ?>" class="btn btn-outline-warning btn-sm flex-fill">Editar</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
