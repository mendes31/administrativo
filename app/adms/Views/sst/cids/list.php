...
$filtersId = 'sstFiltersCid';
?>
<div class="container-fluid px-4">
    <div class="mb-1 hstack gap-2">
        <h2 class="mt-3 mobile-hide-page-title"><i class="fas fa-notes-medical me-2"></i><?= htmlspecialchars('CIDs') ?></h2>
        <ol class="breadcrumb mb-3 ms-auto mobile-hide-breadcrumb">
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>dashboard">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="<?= $_ENV['URL_ADM']; ?>sst-dashboard">SST</a></li>
            <li class="breadcrumb-item"><?= htmlspecialchars('CIDs') ?></li>
        </ol>
    </div>
    <div class="card mb-4 border-light shadow">
        <div class="card-header hstack gap-2">
            <span>Listar</span>
            <span class="ms-auto">
                <?php if (in_array('SstCreateCid', $perms)): ?>
                    <a href="<?= $_ENV['URL_ADM']; ?>sst-create-cid" class="btn btn-success btn-sm"><i class="fa-regular fa-square-plus"></i> Cadastrar</a>
                <?php endif; ?>
            </span>
        </div>
        <div class="card-body">
...
<option value="Inativo" <?= $sel ?>>Inativo</option>
                    ...
                    <div class="col-12 col-sm-auto d-flex gap-2">
                        ...
                        <a href="<?= $_ENV['URL_ADM']; ?>sst-list-cids" class="btn btn-secondary btn-sm">Limpar</a>
                    </div>
                </form>
            </div>
...
<td><?= formatCellValue('descricao', $item['descricao'] ?? null) ?></td>
<td class="text-center"><span class="badge bg-secondary"><?= htmlspecialchars($item['status'] ?? '') ?></span></td>

                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        
                                        <?php if (in_array('SstUpdateCid', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-update-cid/<?= $id ?>" class="btn btn-warning btn-sm"><i class="fa-regular fa-pen-to-square"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstDeleteCid', $perms)): ?>
                                            <form action="<?= $_ENV['URL_ADM']; ?>sst-delete-cid" method="POST" class="d-inline" onsubmit="return confirm('Excluir registro?');">
                ...
                <div class="d-block d-md-none">
                    <?php foreach ($this->data['items'] as $item):
                        $id = (int)($item['id'] ?? 0);
                        $canView = in_array('SstViewCid', $perms);
                        $viewUrl = $_ENV['URL_ADM'] . 'sst-view-cid/' . $id;
                    ?>
                        <div class="card mb-2 shadow-sm"<?php if ($canView): ?> onclick="window.location.href='<?= $viewUrl ?>';" style="cursor:pointer;"<?php endif; ?>>
                            <div class="card-body py-2 px-3">
                                <div class="fw-bold small"><?= formatCellValue('codigo', $item['codigo'] ?? $id) ?></div>
                                <?php if (!empty($item['descricao'])): ?>
                                    <div class="small text-muted"><?= formatCellValue('descricao', $item['descricao']) ?></div>
                                <?php endif; ?>
                                ...
                                <div class="d-flex gap-1 mt-2 pt-2 border-top" onclick="event.stopPropagation();">
                                    ...
                                    <?php if (in_array('SstUpdateCid', $perms)): ?><a href="<?= $_ENV['URL_ADM']; ?>sst-update-cid/<?= $id ?>" class="btn btn-outline-warning btn-sm flex-fill">Editar</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
