...
<div class="container-fluid px-4">
    ...
    <p class="text-muted small">Fila de rascunho S-2210 (CAT) e S-2220 (ASO). JSON de conferência interna — sem envio ao governo. Eventos S-2240 (EPI) e S-2245 (treinamentos) não são gerados.</p>
    <div class="mb-3">
        <?php if (in_array('SstSyncEsocialPendentes', $perms, true)): ?>
            <form method="POST" action="<?= $_ENV['URL_ADM']; ?>sst-sync-esocial-pendentes" class="d-inline" onsubmit="return confirm('Gera apenas rascunhos S-2210 e S-2220. Não transmite ao governo. Continuar?');">
                <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
                <button type="submit" class="btn btn-outline-primary btn-sm"><i class="fas fa-sync"></i> Gerar rascunhos S-2210/S-2220 pendentes</button>
            </form>
        <?php endif; ?>
    ...
    <form method="get" class="row g-2 mb-3 align-items-end">
        <div class="col-md-2"><label class="form-label small">Evento</label><select name="tipo_evento" class="form-select form-select-sm"><option value="">Todos</option>
            <?php foreach (['S-2210', 'S-2220', 'S-2240', 'S-2245'] as $t): ?><option value="<?= $t ?>" <?= ($this->data['filters']['tipo_evento'] ?? '') === $t ? 'selected' : '' ?>><?= $t ?><?= SstEsocialPolicy::isGeracaoBloqueada($t) ? ' (legado)' : '' ?></option><?php endforeach; ?>
        </select></div>
    ...
    <div class="table-responsive">
        <table class="table table-sm table-bordered table-striped">
            ...
            <?php foreach ($this->data['items'] ?? [] as $r): ?>
                <tr>
                    <td><?= (int)$r['id'] ?></td>
                    <td><?= htmlspecialchars($r['tipo_evento'] ?? '') ?><?php if (SstEsocialPolicy::isGeracaoBloqueada((string) ($r['tipo_evento'] ?? ''))): ?> <span class="badge text-bg-secondary">legado</span><?php endif; ?></td>
