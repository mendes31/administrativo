...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4 border-light shadow">
        <div class="card-header hstack gap-2">
            <span>Listar</span>
            <span class="ms-auto">
                <?php if (in_array('SstCreateRisco', $perms)): ?>
                    <a href="<?= $_ENV['URL_ADM']; ?>sst-create-risco" class="btn btn-success btn-sm"><i class="fa-regular fa-square-plus"></i> Cadastrar</a>
                <?php endif; ?>
            </span>
        </div>
        <div class="card-body">
                                ...
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <?php if (in_array('SstViewRisco', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-view-risco/<?= $id ?>" class="btn btn-info btn-sm" title="Visualizar"><i class="fa-regular fa-eye"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstUpdateRisco', $perms)): ?>
                                            <a href="<?= $_ENV['URL_ADM']; ?>sst-update-risco/<?= $id ?>" class="btn btn-warning btn-sm"><i class="fa-regular fa-pen-to-square"></i></a>
                                        <?php endif; ?>
                                        <?php if (in_array('SstDeleteRisco', $perms)): ?>
                                            <form action="<?= $_ENV['URL_ADM']; ?>sst-delete-risco" method="POST" class="d-inline" onsubmit="return confirm('Excluir registro?');">
                        ...
                        <div class="card mb-2 shadow-sm"<?php if ($canView): ?> onclick="window.location.href='<?= $viewUrl ?>';" style="cursor:pointer;"<?php endif; ?>>
                            <div class="card-body py-2 px-3">
                                ...
                                <div class="d-flex gap-1 mt-2 pt-2 border-top" onclick="event.stopPropagation();">
                                    ...
                                    <?php if (in_array('SstUpdateRisco', $perms)): ?><a href="<?= $_ENV['URL_ADM']; ?>sst-update-risco/<?= $id ?>" class="btn btn-outline-warning btn-sm flex-fill">Editar</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
