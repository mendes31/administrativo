...
// Preencher automaticamente departamento e usuário se não pode gerenciar outros
$defaultDepartmentId = $canManageOtherDepartments ? '' : ($currentUser['user_department_id'] ?? '');
$defaultUserId = $canManageOtherDepartments ? '' : ($currentUser['id'] ?? '');
// Cabeçalho já incluso pelo controller
?>
<div class="container-fluid px-4">
    <div class="mb-1 hstack gap-2">
        <h2 class="mt-3">Cadastrar Plano Estratégico</h2>
    ...
    <div class="card mb-4 border-light shadow">
        <div class="card-header">
            <span><i class="fas fa-project-diagram me-2"></i>Cadastrar Plano Estratégico</span>
        </div>
        <div class="card-body">
            <form method="post" action="<?php echo $_ENV['URL_ADM']; ?>create-strategic-plan">
                <div class="row g-3">
                    <div class="col-md-12">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="department_id" id="department_id" class="form-select" required
                                ...
                                <?= $canManageOtherDepartments ? 'onchange="loadUsersByDepartment(this.value)"' : '' ?>>
                            <?php if ($canManageOtherDepartments): ?>
                                <option value="">Selecione o departamento</option>
                            <?php endif; ?>
                            <?php foreach ($departments as $department): ?>
                                <option value="<?= $department['id'] ?>" 
                                        <?= ($plan['department_id'] ?? $defaultDepartmentId) == $department['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-text">
                            <?php if ($canManageOtherDepartments): ?>
                                Departamento responsável pelo plano
                            <?php else: ?>
                                Seu departamento (pré-selecionado)
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="responsible_id" id="responsible_id" class="form-select" required
                            ...
                            <?php foreach ($users as $user): ?>
                                <option value="<?= $user['id'] ?>" 
                                        <?= ($plan['responsible_id'] ?? $defaultUserId) == $user['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-text">
                            <?php if ($canManageOtherDepartments): ?>
                                Pessoa responsável pelo plano
                            <?php else: ?>
                                Você como responsável (pré-selecionado)
                            <?php endif; ?>
                        </div>
                    </div>
