...
<div class="container-fluid px-1 px-md-2 py-0 sales-portal-quotation-compact sales-portal-quotation-layout" id="salesPortalOrderLayoutRoot">

    <div class="hstack gap-1 flex-wrap align-items-center flex-shrink-0 py-0">
        <div class="fw-semibold mb-0 sap-page-title">Pedido de vendas</div>
        <a href="<?= htmlspecialchars($_ENV['URL_ADM'] . 'sales-portal-list-orders' . $connQ, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-outline-secondary btn-sm py-0 px-1">Voltar</a>
        ...
        <ol class="breadcrumb py-0 ms-auto flex-wrap mb-0">
            ...
            <li class="breadcrumb-item"><a href="<?= htmlspecialchars($_ENV['URL_ADM'] . 'sales-portal-list-orders' . $connQ, ENT_QUOTES, 'UTF-8'); ?>" class="text-decoration-none">Pedidos</a></li>
    ...
    <form method="post" action="<?= htmlspecialchars($_ENV['URL_ADM'] . 'sales-portal-save-order', ENT_QUOTES, 'UTF-8'); ?>"
          class="sap-quotation-shell rounded mb-0 overflow-hidden"
          title="POST Orders — campos vazios não enviados; totais no SAP após gravar.">
        ...
        <div class="sap-actions-bar px-2 py-0 d-flex flex-wrap align-items-center gap-2 flex-shrink-0">
            ...
            <a href="<?= htmlspecialchars($_ENV['URL_ADM'] . 'sales-portal-list-orders' . $connQ, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-light btn-sm border-secondary py-0">Cancelar</a>
        </div>
    </form>
</div>
<script>
(function () {
    ...
    function fitOrderPageHeight() {
        var root = document.getElementById('salesPortalOrderLayoutRoot');
        if (!root) { return; }
    ...
    window.addEventListener('resize', fitOrderPageHeight);
    window.addEventListener('orientationchange', fitOrderPageHeight);
    var layoutContent = document.getElementById('layoutSidenav_content');
    if (layoutContent && typeof ResizeObserver !== 'undefined') {
        (new ResizeObserver(function () { fitOrderPageHeight(); })).observe(layoutContent);
    }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () {
            fitOrderPageHeight();
            window.requestAnimationFrame(fitOrderPageHeight);
        });
    } else {
        fitOrderPageHeight();
        window.requestAnimationFrame(fitOrderPageHeight);
