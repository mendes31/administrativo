...
<script>
// Preenche o campo oculto com o nome da categoria selecionada (para compatibilidade com o repositório)
...
function previewImagem(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        label.innerText = 'Clique para fazer upload ou arraste uma imagem aqui';
    }
}
...
function previewAnexo(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        label.innerText = 'Clique para fazer upload ou arraste um arquivo aqui';
    }
}
</script>
...
(function () {
    ...
    wrap.addEventListener('click', function (e) {
        ...
        if (cmd) {
            ...
            // Inline com wrap (mais confiável que execCommand)
            if (inlineMap[cmd]) {
            ...
            // Listas: tenta converter seleção em UL/OL
        ...
        if (action === 'format') {
            // Abre o select de formato (se existir)
            const sel = wrap.querySelector('select[data-action="formatselect"]');
            if (sel) sel.focus();
            return;
        }
    ...
    if (formatSelect) {
        formatSelect.addEventListener('change', function () {
            ...
            // formatBlock recebe tags (p/h1/h2/blockquote)
            editor.focus();
