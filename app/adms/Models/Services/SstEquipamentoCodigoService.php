...
/**
 * Aloca o próximo código de equipamento por tipo (prefixo + sequência de 5 dígitos).
 */
class SstEquipamentoCodigoService extends DbConnection
{
    /**
     * Reserva e devolve o próximo código (ex.: EXT00001). Deve ser chamado dentro de uma transação quando possível.
     *
     * @throws RuntimeException
     */
    public function allocateNextCodigo(int $tipoId, ?PDO $pdo = null): string
    {
        ...
        try {
            $numero = $this->nextNumero($pdo, $tipoId, $prefixo);
    ...
    /**
     * Pré-visualização do próximo código sem consumir a sequência (pode divergir sob concorrência).
     */
    public function peekNextCodigo(int $tipoId): ?string
    {
        $tipo = (new SstEquipamentoTiposRepository())->getById($tipoId);
        if (!$tipo) {
            return null;
        ...
        $stmt = $pdo->prepare(
            'SELECT ultimo_numero FROM adms_sst_equipamento_codigo_seq
             WHERE adms_sst_equipamento_tipo_id = :id LIMIT 1'
        );
        ...
        if ($ultimo === 0) {
            $ultimo = $this->maxNumeroFromEquipamentos($pdo, $tipoId, $prefixo);
        }

        return SstEquipamentoCodigoHelper::format($prefixo, $ultimo + 1);
    }

    private function nextNumero(PDO $pdo, int $tipoId, string $prefixo): int
    {
        $stmt = $pdo->prepare(
            'SELECT ultimo_numero FROM adms_sst_equipamento_codigo_seq
             WHERE adms_sst_equipamento_tipo_id = :id FOR UPDATE'
        );
        $stmt->bindValue(':id', $tipoId, PDO::PARAM_INT);
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if (!$row) {
            $seed = $this->maxNumeroFromEquipamentos($pdo, $tipoId, $prefixo);
            try {
                $insert = $pdo->prepare(
                    'INSERT INTO adms_sst_equipamento_codigo_seq
                        (adms_sst_equipamento_tipo_id, ultimo_numero, updated_at)
                     VALUES (:id, :n, NOW())'
                );
    ...
    private function maxNumeroFromEquipamentos(PDO $pdo, int $tipoId, string $prefixo): int
    {
        $stmt = $pdo->prepare(
            'SELECT codigo FROM adms_sst_equipamentos
             WHERE adms_sst_equipamento_tipo_id = :id AND codigo LIKE :like
