...
/**
 * Gera payloads JSON de conferência para eventos SST do eSocial.
 * Não envia à API do governo. S-2240 (EPI) e S-2245 estão bloqueados (SstEsocialPolicy).
 */
class SstEsocialPayloadService
{
    public const EVENTO_ACIDENTE = SstEsocialPolicy::EVENTO_ACIDENTE;
    public const EVENTO_ASO = SstEsocialPolicy::EVENTO_ASO;
    public const EVENTO_EPI = SstEsocialPolicy::EVENTO_CONDICOES_AMBIENTAIS;
    public const EVENTO_TREINAMENTO = SstEsocialPolicy::EVENTO_TREINAMENTO_LEGADO;

    public function gerarOuAtualizar(string $tipoEvento, string $origemTabela, int $origemId): array
    {
        SstEsocialPolicy::assertGeracaoPermitida($tipoEvento);
    ...
    private function buildS2210(int $acidenteId): array
    {
        ...
        return [
            ...
            'oficial' => false,
            'transmite_governo' => false,
            'layout_referencia' => SstEsocialPolicy::LAYOUT_ATUAL_RASCUNHO,
            'gerado_em' => date('c'),
            ...
            'observacao' => SstEsocialPolicy::observacaoPayload(),
        ];
    }
