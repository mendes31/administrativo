...
class InventorySapSyncService
{
        ...
        try {
            ...
            if (
                ...
                if ($isScopedItem && $filterItemCode !== '') {
                    ...
                    // Full/group sync must revisit every eligible structure, not only pending ones.
                    // Incremental sync without group keeps the previous behavior: pending structures only.
                    $pendingOnlyStructures = !$fullSync && $filterGroupPrefix === '';
                    $structureStats = $this->syncAllItemStructures(
                        $trackRunId,
                        $pendingOnlyStructures,
                        true,
                        $filterGroupPrefix !== '' ? $filterGroupPrefix : null
                    );
                }
    ...
    private function syncStructuresForScopedItem(int $trackRunId, string $erpCode, bool $continueTracking = false): array
    {
        ...
        $unified = $this->syncItemUnifiedByErpCode($erpCode, $trackRunId > 0 ? $trackRunId : null);
        $structStats = is_array($unified['structures'] ?? null) ? $unified['structures'] : [];
        $stats['examined'] = (int)($structStats['examined'] ?? 1);
        $stats['structures_skipped'] = (int)($structStats['skipped'] ?? 0);
        $stats['structures_unchanged'] = (int)($structStats['unchanged'] ?? 0);
        $stats['structures_synced'] = (int)($structStats['synced'] ?? 0);
        $stats['structures_failed'] = (int)($structStats['failed'] ?? 0);

        if ($stats['structures_failed'] > 0) {
            $this->recordSyncFailure(
                $erpCode,
                (string)($unified['message'] ?? 'Falha no sync unificado SAP.'),
                'structures'
            );
        }

        $stats['success'] = !empty($unified['success']);
        $stats['message'] = (string)($unified['message'] ?? '');
     ...
     * }
     */
    public function syncAllItemStructures(
        ?int $trackRunId = null,
    ...
    ): array
    {
        ...
        try {
            ...
            $filterGroupPrefix = trim((string)($filterGroupPrefix ?? ''));
            $ids = $pendingOnly
                ? $itemsRepo->getIdsPendingStructureSync($filterGroupPrefix !== '' ? $filterGroupPrefix : null)
                : $itemsRepo->getIdsEligibleForStructureSync($filterGroupPrefix !== '' ? $filterGroupPrefix : null);
            $total = count($ids);
            if ($total === 0) {
                $stats['success'] = true;
    ...
    /**
     * Catálogo + fecho de dependências (PIs) + estruturas bottom-up + recálculo de custo.
     *
     * @return array<string, mixed>
     */
    public function syncItemUnifiedByErpCode(
        string $erpCode,
        ?int $trackRunId = null,
        bool $recalculateCost = true
    ): array {
        $erpCode = trim($erpCode);
        $result = [
            'success' => false,
        ...
        if ($erpCode === '') {
            $result['message'] = 'Código ERP não informado.';
        ...
        $item = $itemsRepo->findByErpCode($erpCode);
        if (!is_array($item)) {
            $result['message'] = 'Item não encontrado localmente: ' . $erpCode;

            return $result;
        }

        $rootId = (int)($item['id'] ?? 0);
        if (InvCostProjectHelper::isProjectItem($item)) {
            $result['message'] = 'Itens da categoria PA - PROJETO não são sincronizados com o SAP.';

            return $result;
        }

        $catalogResult = $this->syncItemsAndCosts(false, $erpCode, null);
        ...
        foreach ($closure as $code) {
            if (strcasecmp($code, $erpCode) === 0) {
                continue;
            }
            $this->syncItemsAndCosts(false, $code, null);
        }

        foreach ($closure as $index => $code) {
            $localItem = $itemsRepo->findByErpCode($code);
            if (!is_array($localItem)) {
                $result['structures']['failed']++;

                continue;
            }

            $itemId = (int)($localItem['id'] ?? 0);
            if ($itemId <= 0) {
                $result['structures']['failed']++;
            ...
            if ($trackRunId !== null && $trackRunId > 0) {
                $this->publishSyncProgress(
                    [
                        'examined' => $index,
                        'created' => $result['structures']['skipped'],
                        'updated' => $result['structures']['synced'],
                        'unchanged' => $result['structures']['unchanged'],
                        'failed' => $result['structures']['failed'],
                    ],
                    sprintf('Estrutura %s (%d/%d)…', $code, $index + 1, max(1, $closureTotal)),
                    max(1, $closureTotal),
                    true
                );
            }

            $structResult = $this->syncItemStructureById($itemId, false, $sap);
            $result['structures']['examined']++;

            if (!empty($structResult['skipped'])) {
                $result['structures']['skipped']++;
            } elseif (!empty($structResult['unchanged'])) {
                $result['structures']['unchanged']++;
            } elseif (!empty($structResult['success'])) {
                $result['structures']['synced']++;
            } else {
                $result['structures']['failed']++;
            }
        }

        if ($recalculateCost && $rootId > 0) {
            InventoryCostService::recalculateStandardCost($rootId);
        }

        $structures = $result['structures'];
        $result['success'] = !empty($catalogResult['success']) && (int)($structures['failed'] ?? 0) === 0;
        $result['message'] = sprintf(
            'Sync unificado %s: catálogo %s | estruturas %d atualizada(s), %d sem alteração, %d ignorada(s), %d falha(s)%s',
            $erpCode,
            !empty($catalogResult['success']) ? 'ok' : 'com falha',
            (int)($structures['synced'] ?? 0),
    ...
    /**
     * Ordem bottom-up (filhos antes do pai) dos itens com estrutura no fecho SAP.
     *
     * @return list<string>
     */
    private function collectStructureDependencyClosure(string $rootErp, SapReportApiService $sap, int $maxDepth = 8): array
    {
        $rootErp = trim($rootErp);
        if ($rootErp === '') {
            return [];
        }
        ...
        $walk = function (string $erp, int $depth) use (&$walk, &$order, &$visited, &$visiting, $sap, $maxDepth): void {
            if ($depth > $maxDepth || isset($visited[$erp])) {
                return;
            }
            if (isset($visiting[$erp])) {
                return;
            }

            $visiting[$erp] = true;
            $rows = $this->fetchStructureRowsFromSap($sap, $erp);
            [$materialRows] = $this->partitionStructureRowsByTipo($rows);

            foreach ($materialRows as $row) {
                if (!is_array($row)) {
                    continue;
                }
                $childErp = trim((string)($row['codigo'] ?? $row['CODIGO'] ?? ''));
                if ($childErp === '' || !$this->sapMaterialRowEligibleForChildStructure($row, $childErp)) {
                    continue;
                }
                $walk($childErp, $depth + 1);
            }
     ...
     */
    private function sapMaterialRowEligibleForChildStructure(array $row, string $childErp): bool
    {
        $group = (string)($row['component_group_name'] ?? $row['COMPONENT_GROUP_NAME'] ?? '');
        if (InvItemBomExplosionHelper::isIntermediateCategory($group)) {
            return true;
        }

        foreach (['40', '43', '61'] as $prefix) {
            if (str_starts_with($childErp, $prefix)) {
                return true;
            }
        }

        $localItem = (new InvItemsRepository())->findByErpCode($childErp);
        if (!is_array($localItem)) {
            return false;
        }
        ...
        return $bomRepo->getByItem($itemId) !== [] || $opsRepo->getByItem($itemId) !== [];
    }
    ...
    public function syncItemStructureById(int $invItemId, bool $replaceWhenEmpty = true, ?SapReportApiService $sap = null): array
    {
        ...
        try {
            ...
            [$bomLines, $routeLines] = $this->buildStructureLinesFromSapRows(
                $rows,
                $itemsRepo,
                $categoriesRepo,
                $unitsRepo,
            ...
            if ($hashMatch && $linesMatch) {
                ...
                $result['message'] = 'Estrutura idêntica ao SAP e ao cadastro local; nenhuma gravação necessária.';
            ...
            if ($linesMatch) {
                ...
                $result['lines'] = count($bomLines);
                $result['routes'] = count($routeLines);
                $result['message'] = 'Estrutura idêntica ao cadastro local; metadados SAP atualizados.';
                $consolidated = $this->ensureConsolidatedRouteAfterStructure($invItemId);
                $result['consolidated'] = $consolidated['lines'];
                $this->applyConsolidatedRouteSyncResult($result, $consolidated, $invItemId);
    ...
    private function getSapRouteDirectQueryByParentErpCode(string $parentErpCode, bool $includeMasterPosId = true): string
    {
        ...
        $masterSelect = $includeMasterPosId
            ? 'A."MASTER_POS_ID" AS "master_pos_id", '
            : '0 AS "master_pos_id", ';

        return 'SELECT '
            . 'A."POS_ID" AS "pos_id", '
            . 'COALESCE(A."POS_TEXT", A."POS_ID") AS "pos_text", '
            . 'A."SortId" AS "sort_id", '
            . $masterSelect
            . 'A."AG_ID" AS "codigo", '
            . 'A."BEZ" AS "descricao", '
    ...
    /**
     * Rota BEAS: evita CAST no SAP; em erro tenta sem MASTER_POS_ID (API/versão antiga).
     *
     * @return list<array<string, mixed>>
     */
