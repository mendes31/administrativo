...
class SstRiscoTreinamentoRepository extends DbConnection
{
    public function getAll(int $page, int $perPage, array $filters = []): array
    {
        ...
        $sql = "SELECT t.*, r.nome AS risco_nome, tr.nome AS treinamento_nome
                FROM adms_sst_risco_treinamento t
    ...
    public function syncTreinamentosForRisco(int $riscoId, array $treinamentoMap): void
    {
        if ($riscoId <= 0 || !$this->hasTable('adms_sst_risco_treinamento')) {
            return;
        }
            ...
            if (isset($currentByTreinamento[$treinamentoId])) {
                ...
                if ($rowId > 0) {
                    $this->update($rowId, [
                        ...
                        'observacoes' => $currentByTreinamento[$treinamentoId]['observacoes'] ?? null,
                    ]);
                }
    ...
    /** @return list<array<string, mixed>> */
    public function getRiscosByTreinamentoId(int $treinamentoId): array
    {
        if ($treinamentoId <= 0 || !$this->hasTable('adms_sst_risco_treinamento')) {
            return [];
        }
        $sql = 'SELECT DISTINCT r.id, r.nome, r.codigo, r.status, r.grupo_risco, rt.obrigatorio
                FROM adms_sst_risco_treinamento rt
                INNER JOIN adms_sst_riscos r ON r.id = rt.adms_sst_risco_id
                WHERE rt.adms_sst_treinamento_id = :tid
                ORDER BY r.nome';
        $stmt = $this->getConnection()->prepare($sql);
    ...
    {
        $sql = 'INSERT INTO adms_sst_risco_treinamento
                (adms_sst_risco_id, adms_sst_treinamento_id, validade_meses, obrigatorio, observacoes, created_by, updated_by, created_at, updated_at)
                VALUES (:adms_sst_risco_id, :adms_sst_treinamento_id, :validade_meses, :obrigatorio, :observacoes, :created_by, :updated_by, NOW(), NOW())';
        $stmt = $this->getConnection()->prepare($sql);
        ...
        if ($newId > 0) {
            ...
            if ($newData) {
                LogAlteracaoService::registrarAlteracao('adms_sst_risco_treinamento', $newId, $uid, 'INSERT', [], $newData);
            }
        }
    ...
    public function update(int $id, array $data): bool
    {
        ...
        $sql = 'UPDATE adms_sst_risco_treinamento
        ...
        if ($ok && $oldData) {
            ...
            if ($newData) {
                LogAlteracaoService::registrarAlteracao('adms_sst_risco_treinamento', $id, $uid, 'UPDATE', $oldData, $newData);
            }
        }
    ...
    {
        $oldData = $this->getById($id);
        $sql = 'DELETE FROM adms_sst_risco_treinamento WHERE id = :id';
        $stmt = $this->getConnection()->prepare($sql);
        $stmt->bindValue(':id', $id, PDO::PARAM_INT);
        $stmt->execute();
        ...
        if ($deleted && $oldData) {
            $uid = (int) ($_SESSION['user_id'] ?? 1);
            LogAlteracaoService::registrarAlteracao('adms_sst_risco_treinamento', $id, $uid, 'DELETE', $oldData, []);
        }
    ...
    private function buildWhere(array $filters): array
    {
        ...
        if (!empty($filters['search'])) {
            $where[] = '(r.nome LIKE :search OR tr.nome LIKE :search)';
