...
class DepartmentsRepository extends DbConnection
{
    ...
    public function createDepartment(array $data): bool|int
    {
        try {
            ...
            if ($departmentId) {
                ...
                // Invalidar cache de getAllDepartmentsSelect
                $cacheService = new \App\adms\Models\Services\QueryCacheService();
                $cacheService->forget('departments_select_all');
            }
    ...
    public function getAllDepartmentsSelect(): array
    {
        ...
        $stampSql = 'SELECT COUNT(*) AS total_rows, COALESCE(MAX(id), 0) AS max_id FROM adms_departments';
        $stampStmt = $this->getConnection()->prepare($stampSql);
        $stampStmt->execute();
        $stamp = $stampStmt->fetch(PDO::FETCH_ASSOC) ?: ['total_rows' => 0, 'max_id' => 0];
        $cacheKey = 'departments_select_all_' . (int)$stamp['total_rows'] . '_' . (int)$stamp['max_id'];
