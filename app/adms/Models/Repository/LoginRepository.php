...
class LoginRepository extends DbConnection
{
    ...
    public function getUser(string $username): array|bool
    {
        ...
        $sql = 'SELECT t0.id, t0.name, t0.email, t0.username, t0.image, t0.password, t0.user_department_id, t0.user_position_id, t0.created_at, 
                ...
                (SELECT ual.adms_access_level_id FROM adms_users_access_levels ual
                 WHERE ual.adms_user_id = t0.id ORDER BY ual.adms_access_level_id ASC LIMIT 1) AS user_access_level_id
                FROM adms_users t0
    ...
    /**
     * Mesma projeção do login (inclui user_access_level_id) para atualizar a sessão após mudanças no cadastro.
     */
    public function getUserRowForSessionById(int $userId): array|bool
    {
        $sql = 'SELECT t0.id, t0.name, t0.email, t0.username, t0.image, t0.password, t0.user_department_id, t0.user_position_id, t0.created_at,
                t0.updated_at, t0.bloqueado, t0.bloqueado_temporario, t0.data_bloqueio_temporario, t0.tentativas_login, t0.status,
                t0.super_usuario,
