...
class SstCidsRepository extends DbConnection
{
    ...
    {
        if (!in_array('adms_user_id', ['codigo', 'descricao', 'status'], true)) {
            return [];
        }
        $sql = "SELECT t.* FROM adms_sst_cids t WHERE t.adms_user_id = :uid ORDER BY t.id DESC LIMIT :lim";
        $stmt = $this->getConnection()->prepare($sql);
    ...
    {
        $sql = "INSERT INTO adms_sst_cids (codigo, descricao, status, created_by, updated_by, created_at, updated_at)
                VALUES (:codigo, :descricao, :status, :created_by, :updated_by, NOW(), NOW())";
        $stmt = $this->getConnection()->prepare($sql);
    ...
    public function update(int $id, array $data): bool
    {
        ...
        $sql = "UPDATE adms_sst_cids SET codigo = :codigo, descricao = :descricao, status = :status, updated_by = :updated_by, updated_at = NOW() WHERE id = :id";
        $stmt = $this->getConnection()->prepare($sql);
        $stmt->bindValue(':id', $id, PDO::PARAM_INT);
        $this->bindField($stmt, ':codigo', $data['codigo'] ?? null);
        $this->bindField($stmt, ':descricao', $data['descricao'] ?? null);
    ...
    private function buildWhere(array $filters): array
    {
        ...
        if (!empty($filters['search'])) {
            $where[] = '(t.codigo LIKE :search OR t.descricao LIKE :search)';
