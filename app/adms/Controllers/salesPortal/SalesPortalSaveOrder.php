...
/**
 * Grava novo pedido de venda na Service Layer (`POST Orders`).
 *
 * Mesmos cabeçalhos e linhas opcionais que {@see SalesPortalSaveQuotation} (filial, depósito, pagamento, etc.).
 */
class SalesPortalSaveOrder
{
    public function index(): void
    {
        $redirectCreate = $_ENV['URL_ADM'] . 'sales-portal-create-order';
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: ' . $redirectCreate);
            exit;
        }

        if (!CSRFHelper::validateCSRFToken('form_sales_portal_save_order', $_POST['csrf_token'] ?? '')) {
        ...
        if ($docDueDate !== '' && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $docDueDate)) {
            $_SESSION['msg'] = 'Data de vencimento inválida (use AAAA-MM-DD).';
            $_SESSION['msg_type'] = 'warning';
            header('Location: ' . $redirectCreateConn);
            exit;
        }
        ...
        $res = $sl->postResource('Orders', $body);
        if (!empty($res['success'])) {
            ...
            $_SESSION['msg'] = 'Pedido criado com sucesso no SAP B1.';
            $_SESSION['msg_type'] = 'success';
            if ($docEntry > 0) {
                $view = $_ENV['URL_ADM'] . 'sales-portal-view-order?doc_entry=' . $docEntry;
                if ($connectionId > 0) {
        ...
        $_SESSION['msg'] = $res['error'] ?? 'Não foi possível criar o pedido na Service Layer.';
        $_SESSION['msg_type'] = 'danger';
        header('Location: ' . $redirectCreateConn);
        exit;
    }
