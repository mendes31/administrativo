...
class UpdateCompanyEvent
{
    ...
    private function update(int $eventId, CompanyEventsRepository $repo): void
    {
        ...
        if ($repo->updateEvent($eventId, $data)) {
            $_SESSION['msg'] = '<div class="alert alert-success">Evento atualizado.</div>';
            header('Location: ' . $_ENV['URL_ADM'] . 'list-company-events');
            exit;
        }
