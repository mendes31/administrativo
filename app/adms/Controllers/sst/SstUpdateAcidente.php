...
class SstUpdateAcidente
{
    ...
    public function index(string|int|null $id = null): void
    {
        ...
        $this->loadFormData();
        $this->data['anexos'] = (new SstAnexosRepository())->getByEntity('acidentes', (int) $id);
  ...
  array (
    ...
    4 => 'status',
        ...
        $pageElements = [
            'title_head' => 'Update Acidente/Incidente - SST',
            'menu' => 'sst-list-acidentes',
            'buttonPermission' => ['SstUpdateAcidente'],
        ];
    ...
    private function update(int $id): void
    {
        ...
        if ($repo->update($id, $data)) {
            $uploadService->processDeletions($_POST['delete_anexos'] ?? [], 'acidentes', $id);
            $uploadService->processUploads('acidentes', $id);
            $_SESSION['msg'] = 'Registro salvo com sucesso.';
            $_SESSION['msg_type'] = 'success';
            header('Location: ' . $_ENV['URL_ADM'] . 'sst-view-acidente/' . $id);
        } else {
            $_SESSION['msg'] = 'Erro ao salvar registro.';
            $_SESSION['msg_type'] = 'danger';
            header('Location: ' . $_ENV['URL_ADM'] . 'sst-update-acidente/' . $id);
        }
        exit;
    }
}
