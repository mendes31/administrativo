...
/**
 * POST: avaliador convidado recusa participar da entrevista.
 */
class RhEntrevistasRecusarAvaliacao
{
    public function index(int|string $id = 0): void
    {
        ...
        if (!CSRFHelper::validateCSRFToken('form_recusar_avaliacao_entrevista', $csrfToken)) {
        ...
        try {
            $ok = (new RhEntrevistaAvaliadoresRepository())->recusarConvite($entrevistaId, $userId);
            if (!$ok) {
            ...
            header('Location: ' . rtrim((string) ($_ENV['URL_ADM'] ?? ''), '/') . '/rh-entrevistas');
            exit;
        } catch (\Throwable $e) {
            GenerateLog::generateLog('error', 'Falha ao recusar convite de avaliador.', [
                'entrevista_id' => $entrevistaId,
                'user_id' => $userId,
                'error' => $e->getMessage(),
            ]);
            $_SESSION['error'] = 'Erro ao recusar o convite.';
            $this->redirect($entrevistaId);
        }
    }
