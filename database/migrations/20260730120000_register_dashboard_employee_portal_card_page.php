...
/**
 * Card do Dashboard para Portal do Colaborador.
 * ACL alinhada a EmployeePortal (quem acessa o portal vê o card).
 */
final class RegisterDashboardEmployeePortalCardPage extends AbstractMigration
{
    public function up(): void
    {
        ...
        $exists = $this->fetchRow("SELECT id FROM adms_pages WHERE controller = 'DashboardCardEmployeePortal' LIMIT 1");
        if ($exists) {
            return;
        }
        ...
        $refAcl = $this->fetchRow("SELECT id FROM adms_pages WHERE controller = 'EmployeePortal' LIMIT 1");
        if (!$refAcl) {
            $refAcl = $ref;
        }
        ...
        $this->table('adms_pages')->insert([
            'name' => 'Card Dashboard - Portal do Colaborador',
            'controller' => 'DashboardCardEmployeePortal',
            'controller_url' => 'dashboard-card-employee-portal',
            'directory' => 'dashboard',
            'obs' => 'Controla visibilidade do card Portal do Colaborador no dashboard.',
            'public_page' => 0,
            'default_page' => 0,
            'page_status' => 1,
    ...
    public function down(): void
    {
        ...
        $row = $this->fetchRow("SELECT id FROM adms_pages WHERE controller = 'DashboardCardEmployeePortal' LIMIT 1");
        if (!$row) {
            return;
        }
        $pid = (int) $row['id'];
