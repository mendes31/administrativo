...
final class RegisterExtendBookingRecurrencePage extends AbstractMigration
{
    public function up(): void
    {
        ...
        $exists = $this->fetchRow("SELECT id FROM adms_pages WHERE controller = 'ExtendBookingRecurrence' OR controller_url = 'extend-booking-recurrence' LIMIT 1");
        if ($exists) {
            return;
        }

        $ref = $this->fetchRow("SELECT id, adms_groups_page_id FROM adms_pages WHERE controller = 'UpdateBooking' LIMIT 1");
        if (!$ref) {
        ...
        $this->table('adms_pages')->insert([
            'name' => 'Prolongar recorrência de reserva (salas)',
            'controller' => 'ExtendBookingRecurrence',
            'controller_url' => 'extend-booking-recurrence',
            'directory' => 'rooms',
            'obs' => 'POST: adiciona ocorrências semanais até nova data final, sem conflitos.',
            'public_page' => 0,
            'default_page' => 0,
            'page_status' => 1,
    ...
    public function down(): void
    {
        ...
        $row = $this->fetchRow("SELECT id FROM adms_pages WHERE controller = 'ExtendBookingRecurrence' LIMIT 1");
        if (!$row) {
            return;
        }
        $pid = (int)$row['id'];
