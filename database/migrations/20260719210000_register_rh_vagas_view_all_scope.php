...
/**
 * Fase 0.5 Expand — escopo de listagem de vagas (compatível).
 *
 * Cria a página técnica RhVagasViewAll e concede a todos os níveis que já
 * possuem RhVagas, preservando o comportamento atual (ver todas).
 * A retirada seletiva dessa permissão (Contract) fica para decisão posterior.
 */
final class RegisterRhVagasViewAllScope extends AbstractMigration
{
        ...
        if (!$existing) {
            $this->execute(
                'INSERT INTO adms_pages
                 ...
                 VALUES ('
                . $conn->quote('Ver Todas as Vagas (escopo)') . ', '
                . $conn->quote($controller) . ', '
                . $conn->quote('rh-vagas-view-all') . ', '
                . $conn->quote('rh') . ', '
                . $conn->quote('Permissão técnica: listar todas as vagas. Sem ela, a listagem restringe ao responsável.') . ', '
                . '0, 0, 1, 1, '
                . $groupId . ', '
                . $conn->quote($now) . ', '
                . $conn->quote($now)
    ...
    public function down(): void
    {
        ...
        $conn = $this->getAdapter()->getConnection();
        $page = $this->fetchRow(
            "SELECT id FROM adms_pages WHERE controller = 'RhVagasViewAll' LIMIT 1"
        );
        if (!$page) {
            return;
        }
