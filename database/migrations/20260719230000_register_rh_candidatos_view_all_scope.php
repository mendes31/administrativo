...
/**
 * Fase 0.5 Expand — escopo de listagem de candidatos (compatível).
 * Concede RhCandidatosViewAll a quem já tem RhCandidatos.
 */
final class RegisterRhCandidatosViewAllScope extends AbstractMigration
{
    public function up(): void
    {
        ...
        $controller = 'RhCandidatosViewAll';
        ...
        if (!$existing) {
            $this->execute(
                'INSERT INTO adms_pages
                 ...
                 VALUES ('
                . $conn->quote('Ver Todos os Candidatos (escopo)') . ', '
                . $conn->quote($controller) . ', '
                . $conn->quote('rh-candidatos-view-all') . ', '
                . $conn->quote('rh') . ', '
                . $conn->quote('Permissão técnica: listar todos os candidatos. Sem ela, restringe a vínculos com vagas sob responsabilidade.') . ', '
                . '0, 0, 1, 1, '
                . $groupId . ', '
                . $conn->quote($now) . ', '
                . $conn->quote($now)
        ...
        if ($existing) {
            $this->grantPageToRhCandidatosLevels((int) $existing['id']);
        }
    }
    ...
    public function down(): void
    {
        ...
        $conn = $this->getAdapter()->getConnection();
        $page = $this->fetchRow(
            "SELECT id FROM adms_pages WHERE controller = 'RhCandidatosViewAll' LIMIT 1"
        );
        if (!$page) {
            return;
        }
    ...
    private function grantPageToRhCandidatosLevels(int $pageId): void
    {
        if (!$this->hasTable('adms_access_levels_pages')) {
            return;
        }
        ...
        $ref = $this->fetchRow(
            "SELECT id FROM adms_pages WHERE controller = 'RhCandidatos' LIMIT 1"
        );
        if (!$ref) {
            return;
        }
