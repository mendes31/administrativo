...
1. **Tópico da tela** — Atualizar ou criar `content/<modulo>/<topic-id>.html` em português, nesta ordem (**obrigatório em páginas novas**):
   - **`<h2>Função no sistema</h2>`** (logo após o `<h1>`);
   - **`<h2>O que significam os termos</h2>`** — quando houver siglas/jargão ([Plano Diretor](../00_PLANO_DIRETOR/README.md#manual-de-ajuda-e-termos), [Glossário](../02_GLOSSARIO/GLOSSARIO_CORPORATIVO.md));
   - permissões (`Quem acessa`);
   - **`<h2>Fluxo principal</h2>`** — passo a passo;
   - **`<h2>Campos e parâmetros</h2>`** (ou Campos e telas);
   ...
   `php scripts/manual_ensure_structure_sections.php` ·
   `php scripts/manual_structure_audit.php`.
